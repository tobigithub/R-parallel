# R Benchmark 3.0 (10s) (9/2015) [Tobias Kind] roughly scaled to 
# 10 sec each test on 2xE5-2687W@ 3.1 GHz @ Revolution R Pro 3.2.2 (150s total)
...
for (i in 1:runs) {
  invisible(gc())
  timing <- system.time({
    a <- matrix(Rnorm(msize*msize)/10, ncol=msize, nrow=msize);
    b <- t(a);
    dim(b) <- c(msize/2, msize*2);
    a <- t(b)
  })[3]
...
msize=13000
for (i in 1:runs) {
  a <- abs(matrix(Rnorm(msize*msize)/2, ncol=msize, nrow=msize));
  invisible(gc())
  timing <- system.time({ 
    b <- a^1000 
  })[3]
...
timing <- cumulate/runs
times[2, 1] <- timing
cat(c(msize,"x",msize," normal distributed random matrix ^1000_____ (sec): ", timing, "\n"))
remove("a", "b")
if (R.Version()$os == "Win32" || R.Version()$os == "mingw32") flush.console()
...
msize = 12500
for (i in 1:runs) {
  a <- Rnorm(msize*msize); dim(a) <- c(msize, msize)
  invisible(gc())
  timing <- system.time({
...
timing <- cumulate/runs
times[4, 1] <- timing
cat(c(msize,"x",msize," cross-product matrix (b = a' * a)__________ (sec): ", timing, "\n"))
remove("a", "b")
if (R.Version()$os == "Win32" || R.Version()$os == "mingw32") flush.console()
...
msize=11500
for (i in 1:runs) {
  a <- new("dgeMatrix", x = Rnorm(msize*msize), Dim = as.integer(c(msize,msize)))
  b <- as.double(1:msize)
  invisible(gc())
  timing <- system.time({
...
timing <- cumulate/runs
times[5, 1] <- timing
cat(c("Linear regr. over a ",msize,"x",msize," matrix (c = a \\ b')___ (sec): ", timing, "\n"))
remove("a", "b", "c", "qra")
if (R.Version()$os == "Win32" || R.Version()$os == "mingw32") flush.console()
...
msize=2960
for (i in 1:runs) {
  a <- array(Rnorm(msize*msize), dim = c(msize, msize))
...
timing <- cumulate/runs
times[3, 2] <- timing
cat(c("Determinant of a ",msize,"x",msize," random matrix____________ (sec): ", timing, "\n"))
remove("a", "b")
if (R.Version()$os == "Win32" || R.Version()$os == "mingw32") flush.console()
...
msize=19000
for (i in 1:runs) {
  a <- crossprod(new("dgeMatrix", x = Rnorm(msize*msize),
                       Dim = as.integer(c(msize, msize))))
  invisible(gc())
...
timing <- cumulate/runs
times[4, 2] <- timing
cat(c("Cholesky decomposition of a ",msize,"x",msize," matrix________ (sec): ", timing, "\n"))
remove("a", "b")
if (R.Version()$os == "Win32" || R.Version()$os == "mingw32") flush.console()
...
msize=9500000
for (i in 1:runs) {
  a <- ceiling(Runif(msize)*1000)
  b <- ceiling(Runif(msize)*1000)
  invisible(gc())
  timing <- system.time({	  
...
msize=2550
for (i in 1:runs) {
  b <- rep(0, msize*msize); dim(b) <- c(msize, msize)
  invisible(gc())
  timing <- system.time({
...
for (i in 1:runs) {
  x <- abs(Rnorm(msize*msize)); dim(x) <- c(msize, msize)
  invisible(gc())
  timing <- system.time({
...
cat(c("Total time for all 15 tests_____________________________ (sec): ", sum(times), "\n"))
cat(c("Overall mean (sum of I, II and III trimmed means/3)_____ (sec): ", exp(mean(log(times[2:4, ]))), "\n"))
